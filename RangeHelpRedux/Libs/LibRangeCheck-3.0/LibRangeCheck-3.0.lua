...
local lastInitRequestTime = 0
local initInProgress = false
local INIT_DEBOUNCE_DELAY = 1.0 -- seconds
...
local function createCheckerList(spellList, itemList, interactList, batchBudgetMs)
  local res, resInCombat = {}, {}
  local startTime = GetTimePreciseSec()
  batchBudgetMs = batchBudgetMs or 1
  ...
  if itemList then
    for range, items in pairs(itemList) do
      ...
      if batchBudgetMs > 0 and (GetTimePreciseSec() - startTime) * 1000 > batchBudgetMs then
        -- exit early; caller will call createCheckerList again in another batch if desired
        return res, resInCombat, true
      end
    end
  end
  ...
  if spellList then
    for i = 1, #spellList do
      ...
      if batchBudgetMs > 0 and (GetTimePreciseSec() - startTime) * 1000.0 > batchBudgetMs then
        return res, resInCombat, true
      end
    end
  end

  return res, resInCombat, false
end
...
function lib:init(forced)
  -- Debounce: only allow a real init if at least INIT_DEBOUNCE_DELAY seconds
  -- have passed since the last request, unless 'forced' is true.
  local now = GetTime()
  lastInitRequestTime = now
  if self.initialized and not forced then
    -- If initialized, only re-init after the debounce window has fully expired.
    if now - lastInitRequestTime < INIT_DEBOUNCE_DELAY then
      return
    end
  end

  -- mark that a batched init is in progress
  initInProgress = true
  ...
  -- we build a small "plan" of work to run in multiple frames
  if not self._initBatches then
    self._initBatches = {
      { name = "FRIEND RC",        fn = function()
          local list, listIC = createCheckerList(FriendSpells[playerClass], FriendItems, interactList)
          return "friendRC", "friendRCInCombat", list, listIC
        end
      },
      { name = "HARM RC",          fn = function()
          local list, listIC = createCheckerList(HarmSpells[playerClass], HarmItems, interactList)
          return "harmRC", "harmRCInCombat", list, listIC
        end
      },
      { name = "FRIEND NOITEMS RC", fn = function()
          local list, listIC = createCheckerList(FriendSpells[playerClass], nil, interactList)
          return "friendNoItemsRC", "friendNoItemsRCInCombat", list, listIC
        end
      },
      { name = "HARM NOITEMS RC",  fn = function()
          local list, listIC = createCheckerList(HarmSpells[playerClass], nil, interactList)
          return "harmNoItemsRC", "harmNoItemsRCInCombat", list, listIC
        end
      },
      { name = "MISC RC",          fn = function()
          local list, listIC = createCheckerList(nil, nil, interactList)
          return "miscRC", "miscRCInCombat", list, listIC
        end
      },
      { name = "RES RC",           fn = function()
          local list, listIC = createCheckerList(ResSpells[playerClass], nil, interactList)
          return "resRC", "resRCInCombat", list, listIC
        end
      },
      { name = "PET RC",           fn = function()
          local list, listIC = createCheckerList(PetSpells[playerClass], nil, interactList)
          return "petRC", "petRCInCombat", list, listIC
        end
      },
    }
    self._initBatchIndex = 1
    self._initChanged = false
  end

  -- make sure the onUpdate is running while batches are pending
  self.frame:Show()
end
...
function lib:initialOnUpdate()
  ...
  if foundNewItems then
    -- force full rebuild with new items (debounce bypass)
    self:init(true)
    foundNewItems = nil
    return
  end

  -- If we have batched init work, process it in small steps
  if initInProgress and self._initBatches and self._initBatchIndex then
    -- simple spacing: only do real work every ~30ms
    if not self._lastBatchTime or (GetTimePreciseSec() - self._lastBatchTime) > 0.03 then
      local batch = self._initBatches[self._initBatchIndex]
      if batch then
        local name = batch.name
        
        local field1, field2, list, listIC = batch.fn()
        -- list / listIC are already created here; this is the heavy part we spaced out

        local changed = updateCheckers(self[field1], self[field2], list, listIC)
        if changed then
          self._initChanged = true
        end

        self._initBatchIndex = self._initBatchIndex + 1
        self._lastBatchTime = GetTimePreciseSec()
      else
        -- all batches done
        initInProgress = false
        self._initBatches = nil
        self._initBatchIndex = nil

        self.initialized = true
        if self._initChanged and self.callbacks then
          self.callbacks:Fire(self.CHECKERS_CHANGED)
        end
        self._initChanged = nil
        self.frame:Hide()
      end
    end
    return
  ...
  -- No init batches / nothing more to do, hide the frame and unregister item events
  if cacheAllItems then
